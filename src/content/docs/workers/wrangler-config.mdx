...
There is no partial inheritance for bindings or vars. If `[env.preview]` exists, it must list its own D1, R2, KV, AI, services, and `[env.preview.vars]`. A missing binding does not error at deploy — it surfaces as a runtime `undefined` inside the Worker. The one exception is `[assets]` (Static Assets), covered below.
:::
...
Non-binding *settings* such as `workers_dev` and `preview_urls` DO inherit from the top level into named environments. The non-inheritance rule applies to bindings and `[vars]` -- with the single exception of `[assets]` noted above. Setting things explicitly per environment is still common for legibility.
