...
This exists to cut billable Worker invocations for client-heavy SPAs -- most navigations serve `index.html` straight from the edge with zero Worker cost -- not to express a security boundary. If you reach for the array form to "protect" a set of routes, every path outside the list has no gate whatsoever; there is no implicit deny for the paths you didn't list. Use `run_worker_first = true` (or gate inside the Worker itself, as above) when the requirement is authorization. Reserve the array form for routing and cost optimization.
...
### `auto-trailing-slash` Rewrites Before Your Patterns Ever See the Request

The default `html_handling = "auto-trailing-slash"` serves `foo/index.html` **with** a trailing slash, and redirects a bare `/foo` request to `/foo/` first:
...
The trap: any pattern written against the bare form -- a `run_worker_first` array entry, a manual redirect rule, a string comparison in Worker code -- never sees matching traffic, because the browser already followed the 307 before the request you're trying to match arrives. Write patterns against the canonical `/dir/` form:
...
| `run_worker_first` | `false` | The Worker must gate/authorize **every** request -> `true`; a subset of routes needs Worker-first for cost reasons (never security) -> an array of patterns; or the index is SSR with a `dist/404.html` present -> `["/", ...]` |
| `html_handling` | `"auto-trailing-slash"` | Writing `run_worker_first` patterns or redirect rules against a directory path -> use the `/dir/` form the browser actually requests, not the bare `/dir` form it gets redirected away from |
