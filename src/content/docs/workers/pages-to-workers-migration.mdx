...
The build/deploy job split itself follows the same shape as [Production Deploy](../cicd/production-deploy.mdx) -- only the preflight gate in front of it is new, and that self-skip preflight pattern is covered in more depth in [Template Repo CI](../cicd/template-repo-ci.mdx).
