...
| Malformed secret value (e.g. trailing whitespace from a copy-paste) | Re-set the secret via `wrangler secret put` / the GitHub Actions secret |
...
If the CI token's zone permissions keep causing trouble, there's a way to avoid them entirely: keep `workers_dev = false`, declare **no `[[routes]]`** in the committed config, and attach the custom domain once through the dashboard (or a one-time CLI call run outside the pipeline). `wrangler deploy` then never touches the zone API, so the CI token needs zero zone permissions. The tradeoff: attaching the domain becomes a manual, one-time operator step that isn't version-controlled. Treat this as an option alongside the `[[routes]]` path, not a replacement for it.

:::

:::note[Editing a token's permissions keeps the secret; creating a new token doesn't]

Adding a permission to an **existing** token keeps the token string unchanged -- the CI secret needs no update. **Creating a new token** does not preserve the string -- the secret must be re-set via `wrangler secret put` / the GitHub Actions secret. It's easy to grant the missing permission on a freshly created token and then wonder why CI still fails: the pipeline is still authenticating with the old secret value.
...
- [ ] The CI token includes **D1:Edit**, **Vectorize:Read**, and **Workers AI:Read** (plus KV / R2 / Queues / Routes permissions as needed), not just the "Edit Cloudflare Workers" template
