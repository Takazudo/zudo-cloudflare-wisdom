...
| Images (`[images]` binding) | Partial -- `width`, `height`, `rotate`, and `format` only; `fit` / `gravity`, all [composition options](../recipes/worker-image-composition.mdx), and real decoding need a deployment or `--remote` | No |
...
Everything above the Cache API line -- D1, R2, KV, Queues, Rate Limiting, Assets, plain vars, secrets, and the Cache API itself -- emulates locally and needs nothing from Cloudflare's cloud. Below that line, each row is missing something different: Images runs locally without credentials but emulates only a subset of transform options, so anything that depends on real crop or fit behaviour has to be verified against a deployment (see [the Images subset](../recipes/worker-image-preprocessing-blurhash.mdx#runtime-constraints-to-record-before-shipping-the-local-images-binding-is-a-subset-so-verify-fit-and-crop-against-a-deployment); layered composition is entirely absent locally -- see [Layered Image Composition with the Images Binding](../recipes/worker-image-composition.mdx)); AI always needs a live `CLOUDFLARE_API_TOKEN`; Vectorize needs one only if opted into `remote: true`, and is simply absent otherwise; and Workers Cache doesn't run locally at all, regardless of configuration.
