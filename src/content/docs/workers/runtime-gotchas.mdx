...
For a worked decision between synchronous processing, `waitUntil()`, and Queues on one concrete CPU-shaped task -- generating an image placeholder at upload time -- see [Worker-Side Image Preprocessing and BlurHash Placeholders](../recipes/worker-image-preprocessing-blurhash.mdx#synchronous-queues-or-waituntil-pick-by-what-the-response-promises).
