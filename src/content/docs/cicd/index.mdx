---
title: CI/CD
description: GitHub Actions workflows for Cloudflare deployment
sidebar_position: 6
---

All our projects use GitHub Actions to deploy to Cloudflare. This section covers the standard workflow patterns.

## Standard Pipeline

```mermaid
graph LR
  A[Push to main] --> B[Build]
  B --> C[Deploy to CF Pages]
  C --> D[IFTTT Notification]
```

For PRs:

```mermaid
graph LR
  A[Open PR] --> B[Type Check]
  A --> C[Build]
  C --> D[Preview Deploy]
  D --> E[Comment URL on PR]
```

## In This Section

- [Production Deploy](./production-deploy.mdx) -- Main branch deploy workflow
- [PR Preview](./pr-preview.mdx) -- Per-PR preview deployment workflow
- [IFTTT Notifications](./ifttt-notifications.mdx) -- Deploy status notifications
- [Multi-Output Deploy](./multi-output-deploy.mdx) -- Combining multiple builds into one deploy
- [Template Repo CI](./template-repo-ci.mdx) -- Self-skip preflight pattern for template/fork repos with no secrets yet
- [Workers PR Previews](./workers-pr-previews.mdx) -- Per-PR preview deployments for standalone Workers via version aliases
- [Access Service Tokens for CI](./access-service-tokens.mdx) -- Getting Playwright smoke tests and health checks through a Cloudflare Access-protected URL, and keeping credentials straight across two accounts
