...
A bot that receives webhooks (e.g., from Slack), processes them with an external AI API, and replies -- all running on a Cloudflare Worker with KV for state.
...
- **KV for state**: Conversation history and rate limits stored in KV with TTL-based expiration.
...
KV is eventually consistent. Under high concurrency, a few extra requests may slip through. This is acceptable for bot rate limiting -- use Durable Objects if you need precise counting.
:::
