...
None of this is free, though -- it's cheap, not zero, and worth measuring against an actual ceiling rather than just "it's free." The Free plan's CPU-time budget is [10ms per request](https://developers.cloudflare.com/workers/platform/limits/); `buildServer()` plus a handful of synchronous `registerTool()` calls is a small fraction of that on a warm isolate. A `tools/call` that does real work is a different story: an outbound `fetch()`, a KV read, or a heavier registered tool adds its own CPU time on top of the near-free construction cost above, and any outbound call also counts against the Worker's per-invocation subrequest cap. "Cheap to construct" is a claim about the object graph built in this section, not about whatever a specific registered tool goes on to do once `tools/call` reaches it.

## The Request Gate: POST-Only, 405 + Allow Header

The Streamable HTTP transport's own `handleRequest` routes `GET` to an SSE stream and `DELETE` to session termination -- both meaningful only when a session exists. This recipe's transport has no session (`sessionIdGenerator: undefined`), so neither verb has anything to do here; letting the transport handle them anyway means a `GET` or `DELETE` falls through to the transport's own missing-session-id error instead of a clean, predictable response. Gate the method **before** the transport ever sees the request instead: give `OPTIONS` its own branch first -- a browser's CORS preflight arrives as an `OPTIONS` request, and folding it into the generic 405 below would fail every preflight before the browser ever sends the real request, which would quietly break the browser access the "CORS and Auth" section further down describes enabling -- then answer everything else but `POST` the same way:
...
One ordering note for when these pieces are assembled: in the full handler further down, this cap runs *after* the `Content-Type` check covered next, not before -- there is no reason to stream-count a body that was never going to be JSON in the first place. The sections on this page are ordered by concept, not by the sequence the assembled handler actually executes; see [Content-Type and the JSON-RPC Error Shape](#content-type-and-the-json-rpc-error-shape) below for the real order.
...
Neither `server` nor `transport` is explicitly closed anywhere above, and that is deliberate rather than an oversight. Both are function-local objects with no persistent handle attached -- no open socket, no timer, no Durable Object connection -- so there is nothing outside normal garbage collection for a `close()` call to release once `fetch()` returns. The SDK's own Node-based examples close both because a long-lived Node process has to explicitly tear down per-connection state before the next request reuses the same server object; a Worker's `fetch()` handler simply returns and lets its request-scoped objects go, the same way it would for any other function-local value with no external resource attached.
...
This response comes from this recipe's own gate, before the SDK transport ever runs -- confirm that by checking it also fires for `PUT` and `DELETE`, not just `GET`, and that `OPTIONS` gets its own `204` with the same `Allow` header instead of falling into this `405` branch.
