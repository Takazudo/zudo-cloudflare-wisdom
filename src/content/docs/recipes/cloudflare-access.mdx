...
`TEAM_DOMAIN` here is the **full origin** -- `https://my-team.cloudflareaccess.com`, scheme included and no trailing slash. It is used twice for two different reasons: `new URL()` rejects a bare hostname outright, and the `iss` claim Access signs is the origin form, so a value that omits the scheme fails the `issuer` check even where the URL happens to build.
