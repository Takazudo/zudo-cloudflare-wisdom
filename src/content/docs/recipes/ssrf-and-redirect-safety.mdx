...
| `0.0.0.0/8`, including bare `http://0/` | IPv4 unspecified address -- on many networking stacks this resolves locally, to "this host," the same class of target as loopback. `new URL()` normalizes the bare literal `0` to `0.0.0.0`, so `http://0/` reaches this range without ever writing a dotted-decimal IP |
...
| `::1` | IPv6 loopback |
| `::` | IPv6 unspecified address -- the IPv6 analogue of `0.0.0.0` above |
...
### Cross-Origin Hops Must Not Carry the Original Credentials or Body

`redirect: "manual"` does not just make the redirect visible -- it also opts the guard out of everything native `fetch()` normally does when it follows a redirect for you: stripping `Authorization` and `Cookie` before crossing to a different origin, and downgrading a `301`/`302`/`303` to a bodyless `GET` when the original method was neither `GET` nor `HEAD`. A guard that re-fetches each hop by hand has to reimplement both, or `init` -- whatever the caller passed in, credentials and body included -- gets replayed unchanged against every hop, including one an attacker chose. Skipping this is exactly how a validated webhook URL that 302s to `https://attacker.example/collect` hands the attacker the caller's own `Authorization` header or session cookie.
...
:::tip[The alternative is rejecting cross-origin redirects outright]
This recipe rebuilds the request instead of refusing to follow a cross-origin hop at all, because a link-preview generator or webhook-target guard often has a legitimate reason to follow a redirect onto a different host -- a URL shortener, a CDN, a signed-URL redirect -- and rejecting those outright would make the guard reject links that were never a threat. The cost is silence: a caller that actually needed `Authorization` or `Cookie` to reach the cross-origin target gets whatever unauthenticated response that origin returns, not an error from this guard. If the legitimate cross-origin destinations are known ahead of time, rejecting any redirect outside that allowlist -- the same allowlist argument made for the blocklist itself, above -- is the stronger choice.
:::
...
Every hop -- including the first -- goes through the exact same `assertNotBlocked()`, so there is only one place the blocklist logic lives. A cross-origin hop additionally goes through `rebuildForRedirect()`, so the credentials the caller sent to the origin it explicitly chose never travel further than that origin.
...
### Decode Exactly Once -- and That Decode Already Happened Upstream

A check that scans the **raw, still-encoded** query value never sees the dangerous bytes -- `%0d%0a` reads as four harmless ASCII characters until it is decoded into an actual CR/LF pair. Validation has to run on the decoded string, decoded **exactly once**: never decode in a loop until the string stops changing. Repeated decoding lets the attacker choose how many layers deep the real payload sits -- a value like `%250d%250a` should decode once to the literal four characters `%0d%0a` (harmless, no control bytes present) and stay that way; decoding it a second time turns it into a real CR/LF.

That one required decode already happens before `sanitizeRedirectTarget` ever sees the value: `URLSearchParams.get()` (a query parameter) and `request.formData().get()` (a POST field, as in [Password Gate](./password-gate.mdx#login-is-post-only-with-a-fixed-delay-on-failure)) both percent-decode their input once, as part of parsing the query string or form body. `sanitizeRedirectTarget` must treat what it receives as that one decode's result and validate it directly -- it must not call `decodeURIComponent()` on it again. Doing so is the same bug as the decode-loop above, just introduced by the sanitizer itself instead of a caller.

Concretely: a destination whose own query string carries an encoded value -- say `/search?q=a%26role%3Dadmin`, where `%26` and `%3D` are that destination's own escaped `&` and `=` -- has to arrive double-escaped in the outer `next` parameter (`%2526`, `%253D`) to survive the trip through the outer query string. `URLSearchParams.get()` performs its one decode and hands `sanitizeRedirectTarget` exactly `/search?q=a%26role%3Dadmin`, the destination's own encoding intact. A second `decodeURIComponent()` call inside the sanitizer decodes that too, turning it into `/search?q=a&role=admin` -- a second query parameter, `role=admin`, injected into the URL the sanitizer hands back, that was never part of the original destination.
...
| As written in the URL's query string | As received by `sanitizeRedirectTarget` (after `URLSearchParams`' one decode) | What happens |
| --- | --- | --- |
