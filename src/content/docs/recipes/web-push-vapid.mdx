...
`hkdf`'s parameters are typed `BufferSource` -- the union `crypto.subtle.importKey()` and `deriveBits()` actually accept -- rather than the narrower `ArrayBuffer`. Every call site above passes a `Uint8Array` (`authSecret`, `ikmInfo`, the `TextEncoder` output, the random salt from `crypto.getRandomValues`), and under `strict` TypeScript a `Uint8Array` is not assignable to a bare `ArrayBuffer` parameter -- typing `hkdf` as `ArrayBuffer` would fail to compile the moment any of these call sites ran through `tsc`, even though every one of them works correctly at runtime.

:::note[Push payloads are small on purpose]
RFC 8030 §5.2 requires a push service to **support at least** 4096 octets of
encrypted content -- a floor, not a cap -- but in practice most push services
enforce something close to that floor as their real ceiling anyway, so
designing for it is the safe default. A single `aes128gcm` record's overhead
...
### Validate the Endpoint Host at Subscribe Time

`redirect: "manual"` above closes the redirect hop, but it does nothing about
the *initial* request: `subscription.endpoint` is whatever the client's
subscribe request said it is, and nothing forces that it actually came from a
real `PushManager.subscribe()` call. Anyone who can reach your subscribe
endpoint can submit any URL there, including an internal service or a cloud
metadata endpoint, and the server will dutifully sign a VAPID JWT and `POST`
an encrypted body to it -- the same outbound-fetch risk
[SSRF and Redirect Safety](./ssrf-and-redirect-safety.mdx) covers in general,
applied to a URL that arrives through a subscription instead of a redirect.

The real push services are a short, stable list of hosts, so validate
against an allowlist at subscribe time rather than trying to block the ones
you don't want -- the same allowlist-over-blocklist framing
[SSRF and Redirect Safety](./ssrf-and-redirect-safety.mdx) uses for outbound
fetches generally:
...
Run this check when a subscription is first submitted, before it is ever
stored -- rejecting a bad `endpoint` at subscribe time means `sendPushMessage`
...
- **`404` or `410`:** the subscription is permanently gone -- the user
  ...
  the subscription from storage -- [KV](../storage/kv.mdx) or
  [D1](../storage/d1.mdx), depending on where subscriptions live -- on this
  response, not on some accumulated failure count.
...
## Fan-Out: Subrequest Caps and Reusing the VAPID JWT

The natural way to notify many subscribers is a loop that calls
`sendPushMessage` once per subscription -- which means one outbound
`fetch()` per subscription, against Workers' per-invocation subrequest cap:
**50 on the Free plan, 10,000 by default on Paid** (the Paid limit is
configurable up to 10 million; see
[Limits](https://developers.cloudflare.com/workers/platform/limits/) for the
current numbers). A notification to a mailing list large enough to exceed
whichever cap applies blows through it from a single invocation, and the
failure looks like a partial, seemingly random drop in delivery rather than
an obvious error.

`sendPushMessage` also signs a fresh VAPID JWT for every single message, and
that is wasteful for exactly the same fan-out. `signVapidJwt`'s claims
depend only on `audienceOrigin` -- the push endpoint's origin -- and
`VAPID_EXP_SECONDS` above is a full hour. A fan-out to a few thousand
subscribers realistically touches only a handful of distinct origins (FCM,
Mozilla's autopush, Apple's web push service, and whatever else the
subscriber base uses), so signing per message repeats the same ECDSA
signature dozens or hundreds of times for an origin whose JWT is already
valid for the next hour. Sign once per origin and reuse it across every
subscription that shares it:
...
### Chunking Beyond the Subrequest Cap

`sendPushBatch` above still issues one `fetch()` per subscription from a
single Worker invocation, so it is only safe up to the subrequest cap
itself. A subscriber list larger than that has to be split across multiple
invocations, not sent from one:

- `ctx.waitUntil(sendPushBatch(chunk, payload, env))` is worth reaching for
  regardless of list size: it lets the triggering request return
  immediately instead of the caller waiting on every push delivery, since
  nothing about triggering a notification fan-out needs the client to block
  on it. It does not, on its own, raise the subrequest cap -- that cap
  applies for the life of one invocation whether the fetches happen
  synchronously in the handler or deferred into a `waitUntil`.
- The only way to send more messages in total than the cap allows for a
  single invocation is to split the list across multiple invocations.
  [Cloudflare Queues](https://developers.cloudflare.com/queues/) is the
  standard shape for that: enqueue one message per chunk of subscribers
  sized to fit inside the cap, and let each queue consumer -- itself a
  fresh Worker invocation with its own subrequest budget -- call
  `sendPushBatch` on that chunk. Queues' built-in retry is also a natural
  fit here: a `transient` outcome for one subscriber in a chunk shouldn't
  fail the whole chunk's delivery.
...
For local development, put the same JSON string in `.dev.vars` (gitignored,
never committed) instead of running `wrangler secret put` against a local
environment:
...
:::danger[Never declare a secret's name in `[vars]` too]
`VAPID_PRIVATE_KEY_JWK` is typed by the `Env` interface declared earlier on
this page -- `[vars]` is not a typing mechanism, and it never needs a blank
placeholder entry for a name that's set as a secret. A Worker has one
binding per name, so declaring the same name in both `[vars]` and as a
secret is a collision, not a harmless redundancy: reports against
`wrangler deploy` describe it silently overwriting a dashboard- or
CLI-set secret value with whatever `[vars]` says at deploy time
...
At best the precedence between the two is undocumented; at worst the next
deploy silently replaces a working VAPID private key with an empty string,
and every push starts failing with an opaque push-service rejection.
Leaking the real secret value, wherever it ends up set, is equivalent to
leaking every VAPID-signed push identity for this Worker at once -- the same
blast radius as the shared signing keys covered in
[HTTP-only Cookie Sessions](./cookie-sessions.mdx) and
[Signing Webhooks with HMAC-SHA256](./webhook-signing.mdx).
:::

:::warning[The example `VAPID_PUBLIC_KEY` above is a placeholder, not a usable key]
Generate your own with `generateVapidKeyPair()` from "Generating and Storing
the VAPID Key Pair" above. Do not reuse the public key from the RFC 8291
known-answer test vector earlier on this page -- it's the **ephemeral**
ECDH public key from that vector, whose matching private key is printed two
rows above it and published worldwide in the RFC. Copying it into a real
config mixes up the two distinct P-256 key pairs this page is careful to
keep separate (the long-lived VAPID **ECDSA** identity vs. the per-message
ephemeral **ECDH** key) and ships a public key with a globally known private
half.
:::
