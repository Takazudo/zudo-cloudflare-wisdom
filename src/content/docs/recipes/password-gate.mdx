...
See [Declaring Required Secrets](../workers/wrangler-config.mdx#declaring-required-secrets) for the full dev / first-deploy / redeploy matrix.
...
- [Wrangler Config: Declaring Required Secrets](../workers/wrangler-config.mdx#declaring-required-secrets) -- the `[secrets] required` checklist that turns an unbound `GATE_PASSWORD_HASH` into a deploy-time error instead of a runtime 500.
