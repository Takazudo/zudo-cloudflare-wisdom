...
:::tip[Hash-first, timing-safe compare]
`timingSafeEqualHex` never returns early on a byte mismatch -- only
`crypto.subtle.timingSafeEqual()`'s constant-time comparison decides that. A
naive `===`, by contrast, returns the moment the two strings diverge, and an
attacker who can measure response timing precisely enough can use that to
recover the correct signature one byte at a time.
`crypto.subtle.timingSafeEqual()` is the Workers runtime's own primitive for
this comparison rather than a hand-written loop -- ECMAScript makes no
constant-time guarantee for any particular piece of JS, so a JIT-optimized
loop is not a safe substitute. This is the same hash-first, timing-safe shape
used throughout this site's other secret-comparison recipes -- see
[Password Gate](./password-gate.mdx#verifying-the-password-hash-first-then-compare-in-constant-time)
for the same technique applied to a variable-length secret (a password)
instead of an already fixed-length digest.
:::
