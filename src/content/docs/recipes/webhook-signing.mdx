...
`timingSafeEqualHex` never returns early on a byte mismatch -- only the loop's
constant-time XOR decides that. A naive `===`, by contrast, returns the moment
the two strings diverge, and an attacker who can measure response timing
precisely enough can use that to recover the correct signature one byte at a
time. This is the same hash-first, timing-safe shape used throughout this
site's other secret-comparison recipes -- see [Password Gate](./password-gate.mdx#verifying-the-password-hash-first-then-compare-in-constant-time)
