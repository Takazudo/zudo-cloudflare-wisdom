...
:::note[Cloudflare Access needs nothing on this token]
Putting a site behind [Cloudflare Access](../recipes/cloudflare-access.mdx) does not change the deploy token. Access configuration is a separate API surface with its own account-scoped `Access: *` permissions, and none of them belong here -- adding Access to a project never breaks an existing pipeline, and the deploy token stays as narrow as the list above.
:::
