...
約 100 MB を超えるファイルには、マルチパートアップロードを使用する：
...
R2 バケットはデフォルトで非公開だ。ファイルを公開配信するには：

1. **カスタムドメイン**：Cloudflare ダッシュボードでバケットにドメインを接続する
2. **Worker プロキシ**：R2 から読み取り、ファイルを配信する Worker を作成する
3. **Pages Function**：ファイル配信エンドポイントとして Pages Function を使用する
...
[`aws4fetch`](https://github.com/mhart/aws4fetch)（minify 後 ~5 KB）を使い、`@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` は **使わない**。AWS SDK v3 は重い smithy / `AbortSignal` の仕組みを同梱しており、ここでは単一の署名呼び出しのためだけに数百 KB の Worker バンドルを抱え込む。かつての **1 MB** の Worker サイズ上限は丸ごと突破していた。現在の上限は執筆時点で **Free プランが圧縮後 3 MB、Paid が 10 MB** なので、致命的な失敗ではなく無駄になったが、それでも同梱する理由にはならない。
...
行にバイト列から導出したメタデータ——寸法や BlurHash プレースホルダ——も載せる場合は、それを `put()` の**前に**計算する。導出に失敗しても孤立オブジェクトは残らず、行の INSERT に失敗したときはオブジェクトをベストエフォートで削除すればよい。[Worker 側の画像前処理と BlurHash プレースホルダ](../recipes/worker-image-preprocessing-blurhash.mdx) が、リトライの意味論も含めてこの取り込み順序の全体を追っている。
...
- **メタデータ**: 独自のキーバリューペアには `customMetadata`、HTTP ヘッダーには `httpMetadata` を使用。一覧ページがオブジェクトを取得する*前に*必要になる値（プレースホルダのハッシュ、寸法）は D1 のカラムが読み取り経路であり、`customMetadata` はせいぜい二次的なコピーにすぎない——[D1 のカラムか R2 の customMetadata か](../recipes/worker-image-preprocessing-blurhash.mdx#d1-のカラムか-r2-の-custommetadata-か-読み取り経路に従う) を参照
