...
description: ダウンタイムなしでライブサイトを Cloudflare Pages から Workers Static Assets へ移し、うまくいかなかった場合にきれいにロールバックする
sidebar_position: 10
---

## なぜ移行するのか

[Workers Static Assets](./static-assets.mdx) は、静的サイトと SSR サイトにおける Cloudflare Pages の後継だ -- 別個の Pages プロジェクトとそのデプロイパイプラインを持つ代わりに、アセットディレクトリとリクエストロジックの両方を 1 つの Worker が持つ。ゼロから作るならこれを直接使えばいい。このページが扱うのは別のケースだ：すでに Pages でライブ稼働しているサイトを、サービスの空白なしで Workers に移したい場合。
...
5. **DNS/ドメインの切り替え。** 実トラフィックが動くのはこのステップだけだ -- 詳細は後述する。
...
Zone の行が現れるのは、カスタムドメインやルートをアタッチする場合だけだ。理由は、Workers のカスタムドメインとルートがゾーンスコープのリソースであるのに対し、Pages のカスタムドメインはアカウントスコープの Pages API 経由で管理されており、そもそも Zone の権限カテゴリに一度も触れていなかったからだ。サイトが `workers.dev` の URL だけで完結するなら、真ん中の列で止めていい -- それで完了だ。Pages サイトが現在応答しているのと同じカスタムドメインが必要なら、Zone の行のぶんの予算は切り替えステップの最中ではなく、その前に確保しておくこと。
...
リポジトリ変数 `WORKERS_MIGRATION_ACTIVE` が `true` にセットされるまでは、`preflight` は実行されて `active=false` と評価され、`deploy` は失敗ではなく**スキップ**として報告される。これにより、移行期間の全体を通じて、ワークフローは push のたびにマージ済み・構文チェック済み・シークレット解決済みの状態を保ちながら、誤って何かをデプロイしてしまうことはない。トークンを更新して最初の手動デプロイを確認し終えたら、この変数を切り替えるだけでいい。同じ、すでに PR でテスト済みのワークフローがそのまま本番デプロイを始める -- 初めて信頼しなければならない別の「有効化」コードパスは存在しない。
