...
これが存在するのは、クライアント重めの SPA で課金対象の Worker 呼び出しを減らすためだ -- ほとんどのナビゲーションは Worker のコストゼロでエッジから直接 `index.html` を配信する -- セキュリティ境界を表現するためではない。ルート集合を「保護する」つもりで配列形式に手を伸ばすと、リストの外にあるパスにはゲートが一切かからない。列挙しなかったパスに対する暗黙の拒否は存在しない。認可が要件なら `run_worker_first = true`（あるいは前述のように Worker 内部でゲートする）を使うこと。配列形式はルーティングとコスト最適化のために取っておく。
...
### `auto-trailing-slash` はパターンがリクエストを目にする前に書き換えてしまう

デフォルトの `html_handling = "auto-trailing-slash"` は `foo/index.html` をトレイリングスラッシュ**あり**で配信し、素の `/foo` リクエストはまず `/foo/` へリダイレクトする：
...
罠はこうだ。素の形に対して書いたパターン -- `run_worker_first` の配列エントリ、手動のリダイレクトルール、Worker コード内の文字列比較 -- は一致するトラフィックを一切目にしない。あなたがマッチさせようとしているリクエストが到着する前に、ブラウザはすでに 307 をたどっているからだ。正規形である `/dir/` の形に対してパターンを書くこと：
...
binding = "ASSETS"
not_found_handling = "single-page-application"
# Wrong: "/admin" never arrives here -- auto-trailing-slash 307s it to
# "/admin/" first. Match the form that's actually requested.
...
| `run_worker_first` | `false` | Worker が**すべての**リクエストをゲート・認可する必要がある -> `true`；一部のルートだけコスト上の理由で Worker-first にしたい（セキュリティ目的では使わない）-> パターンの配列；あるいはインデックスが SSR で `dist/404.html` が存在する -> `["/", ...]` |
| `html_handling` | `"auto-trailing-slash"` | `run_worker_first` のパターンやリダイレクトルールをディレクトリパスに対して書く -> ブラウザが実際にリクエストする `/dir/` の形を使うこと。リダイレクトされて消えていく素の `/dir` の形ではなく |
