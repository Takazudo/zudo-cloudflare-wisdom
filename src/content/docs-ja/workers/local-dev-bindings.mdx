...
Cache API までのブロック -- D1、R2、KV、Queues、Rate Limiting、Assets、プレーンな vars、secrets、そして Cache API 自体 -- はすべてローカルでエミュレートされ、Cloudflare のクラウド側からは何も必要としない。それより下の行はそれぞれ足りないものが違う：Images はクレデンシャルなしでローカル動作するが、変換オプションのサブセットしかエミュレートしないため、本物のクロップや fit の挙動に依存するものはデプロイ先で検証する必要がある（[Images のサブセット](../recipes/worker-image-preprocessing-blurhash.mdx#出荷前に記録しておくべきランタイム制約-ローカルの-images-バインディングはサブセットなので-fit-とクロップはデプロイ先で検証する) を参照。レイヤー合成にいたってはローカルにまったく存在しない —— [Images バインディングによるレイヤー合成](../recipes/worker-image-composition.mdx) を参照）。AI は常に有効な `CLOUDFLARE_API_TOKEN` を必要とする。Vectorize は `remote: true` を選んだ場合だけそれを必要とし、そうでなければ単に存在しない。そして Workers Cache は、設定にかかわらずローカルでは一切動かない。
