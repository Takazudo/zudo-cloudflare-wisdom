...
:::warning[このパターンは 1 対 1 のストリーム向けで、ファンアウトには使えません]
上記の `AbortController` と `signal` / `cancel()` の組み合わせが機能するのは、
クライアントが 1 つ、ストリームも 1 つだからです — 中断を行う場所も、破棄すべき
ストリームも、ちょうど 1 つしかありません。これはプレーンな Worker であっても
Durable Object であっても、1 対 1 のストリーミングハンドラであれば成り立ちます。
DO の外で動かしていることに固有の話ではありません。

このパターンが崩れるのはファンアウトのとき、あるいはアイドル状態になりうる
ストリームのときです。ランタイムがその特定の接続に対して実際に I/O を試み、
それが失敗しない限り `signal` も `cancel()` も発火しないため、多数の接続へ
配信するブロードキャストループでは、閉じられたクライアントが何分も検知され
ないまま残ることがあります。その典型例が、Durable Object が SSE を多数の
接続へファンアウトするケースです。なぜそこではリクエストごとの中断だけでは
不十分なのか、そしてそれに代わるハートビートのパターンについては
[SSE ファンアウトとハートビートによる切断検知](../workers/durable-objects.mdx#sse-ファンアウトとハートビートによる切断検知)
を参照してください。
:::
