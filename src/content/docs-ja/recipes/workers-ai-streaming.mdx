...
:::warning[このパターンはプレーンな Worker 専用]
上記の `AbortController` と `signal` / `cancel()` の組み合わせが機能するのは、
このレシピが 1 度に 1 つのリクエストだけを扱う単一の Worker だからです —
中断を行う場所も、破棄すべきストリームも、ちょうど 1 つしかありません。
一度に多数の接続へ SSE をファンアウトする Durable Object の内部では、
同じフックは信頼できる切断シグナルにはなりません。理由と、それに代わる
ハートビートのパターンについては
