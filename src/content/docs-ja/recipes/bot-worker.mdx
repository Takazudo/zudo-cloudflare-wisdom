...
上記の `a.length !== b.length` は、一定時間ループが走る前にリターンする。これが安全なのは、`hexSig` と `signature` がどちらも `v0=` に固定 64 文字の SHA-256 16 進ダイジェストが続く形であることを前提にしているためだ -- 正しい長さは署名シークレットから導かれるものではなく公開情報なので、長さの不一致による早期リターンは攻撃者がすでに知っていること以上の何も明かさない。この前提は、比較のどちらか一方が公開された長さのダイジェストではなく可変長のシークレットになった瞬間に崩れる -- たとえば送信されたパスワードを直接比較すると、その同じ早期リターンを通じて本物のパスワードの長さが漏れてしまう。可変長のシークレットはまず固定長のダイジェストへハッシュ化してから、その結果に対して一定時間比較を行うこと -- 同じ比較をパスワード検証に適用したハッシュ優先版については [パスワードゲート](./password-gate.mdx#パスワードの検証-まずハッシュ化し、それから一定時間で比較する) を参照。
:::
