...
:::tip[タイミングセーフな比較を使う]
署名検証には必ず定数時間の比較を使う -- 具体的には、まさにこのために Workers ランタイムが自ら提供するプリミティブである `crypto.subtle.timingSafeEqual()` を使う。通常の `===` や、見た目だけ定数時間に見える自前の比較ループは、どちらもタイミングサイドチャネルで情報が漏洩する可能性がある。ECMAScript は自前のループに対してそのような保証を一切与えない。
:::
...
:::tip[早期の長さチェックはここでは何も漏らさない -- HMAC-SHA256 のダイジェスト長は公開情報であってシークレットではない]
上記の `a.length !== b.length` は、`crypto.subtle.timingSafeEqual()` が走る前にリターンする。これが安全なのは、`hexSig` と `signature` がどちらも `v0=` に固定 64 文字の SHA-256 16 進ダイジェストが続く形であることを前提にしているためだ -- 正しい長さは署名シークレットから導かれるものではなく公開情報なので、長さの不一致による早期リターンは攻撃者がすでに知っていること以上の何も明かさない。これは必須でもある: `crypto.subtle.timingSafeEqual()` は、2 つのエンコード済みバッファの長さが異なると false を返すのではなく例外を投げる。この前提は、比較のどちらか一方が公開された長さのダイジェストではなく可変長のシークレットになった瞬間に崩れる -- たとえば送信されたパスワードを直接比較すると、その同じ早期リターンを通じて本物のパスワードの長さが漏れてしまう。可変長のシークレットはまず固定長のダイジェストへハッシュ化してから、その結果に対して一定時間比較を行うこと -- 同じ比較をパスワード検証に適用したハッシュ優先版については [パスワードゲート](./password-gate.mdx#パスワードの検証-まずハッシュ化し、それから一定時間で比較する) を参照。
:::
