...
RFC 8030 はプッシュメッセージの暗号化済みコンテンツを 4096 オクテットに制限して
おり、プッシュサービスもほぼその上限を強制する。単一の `aes128gcm` レコードの
...
[SSRF とリダイレクトの安全性](./ssrf-and-redirect-safety.mdx#ホップごとのリダイレクト再チェック)
...
- **`404` または `410`:** サブスクリプションは永続的に失われている -- ユーザーが
  ...
  蓄積された失敗回数ではなく、このレスポンスを受けた時点でサブスクリプションを
  ストレージから削除すること。
...
:::danger[Secret と Vars]
`[vars]` の中の空の `VAPID_PRIVATE_KEY_JWK` は、ローカルでの型付けのためだけの
プレースホルダーだ -- 実際の値は `wrangler secret put VAPID_PRIVATE_KEY_JWK` で
設定し、決してコミットしない。これが漏れることは、この Worker に紐づくすべての
VAPID 署名済みプッシュの身元を一度に漏らすことに等しく、[HTTP-only Cookie セッション](./cookie-sessions.mdx)
や [HMAC-SHA256 によるウェブフック署名](./webhook-signing.mdx)で扱った共有署名鍵と
同じ被害半径を持つ。
:::
