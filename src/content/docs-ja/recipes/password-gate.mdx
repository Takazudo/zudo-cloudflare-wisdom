...
空の `[]` は、リポジトリ設定画面が何を表示していようと、その Worker にシークレットが 1 つもバインドされていないことを意味する。再デプロイしても直らない: `wrangler deploy` はシークレットの*値*を書き込まず、前バージョンが持っていたものをそのまま引き継ぐよう API に依頼するだけなので、そもそも Cloudflare のストアに一度も入れていない名前は、エラーも出ないまま存在しないままになる。wrangler に強制させたい名前を宣言しておけば、この静かな実行時の `undefined` を、デプロイ時のエラーに変えられる:
...
| --- | --- |
| `GATE_PASSWORD_HASH`（下記のガードがない場合） | `timingSafeEqualHex` が `undefined` に対して `b.length` を読み、`TypeError: Cannot read properties of undefined (reading 'length')` を投げる -- 500 になり、ログインは 1 つも通らない |
...
どちらも拒否しており、結果としては正しい。ただし `GATE_PASSWORD_HASH` の拒否は判断の結果ではなく、ハンドルされない例外の副作用にすぎない。そこで下の `verifyPassword` は、その契約を明示するガードから始めている。`GATE_SECRET` には手書きの相当物は不要だ: ランタイムが返す長さ 0 の鍵に対する `DataError` は、値に依存しない決定的なハードエラーだからだ。どちらの場合にも必要なのは、次にこのコードを読む人がその拒否 -- ハッシュ側はガードによる `401`、`GATE_SECRET` 側は `importKey` の 500 -- を「潰すべきバグ」ではなく「ゲートが機能している証拠」として認識することだ。

:::danger[コミットされたフォールバックは、フェイルクローズをフェイルオープンに変える]
どちらの拒否に対しても誘惑的なパッチが、デフォルト値だ:
...
ローカル用の値は `.dev.vars` に置く。`wrangler dev` はそこの値を本物のシークレットとしてバインドし、このファイルはコミットもデプロイもされない。ハードコードされたフォールバックがどうしても避けられない場合は、リクエストが影響を与えられないもの -- 本番バンドルから削り落とされるビルド時フラグなど -- を条件にすること。リクエスト自身のホスト名を条件にしてはいけない: `request.url` と `Host` ヘッダーはクライアントが指定するものであり（[SSRF とリダイレクト安全性](./ssrf-and-redirect-safety.mdx) を参照）、サービスバインディングや別 Worker の `fetch()` 経由で呼ばれた Worker は `localhost` を含む任意のホスト名を持ちうる。そもそも `.dev.vars` を使えばフォールバック自体が不要になる。このやり方については [ローカル開発とバインディング](../workers/local-dev-bindings.mdx) を参照。そこには、認証ゲートが `undefined` を読んで「無効」に解決してしまい、テストが `401` を期待した場所で `200` が返るという、同じ失敗の process-env 版も載っている。
:::
