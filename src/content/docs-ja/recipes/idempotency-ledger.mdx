...
このレシピは、この契約のサーバー側を D1 の上に構築する。キーをアトミックに claim する台帳テーブルを用意し、通常の条件下では mutation をキーごとに 1 回だけ実行し、キーがまだ保持されている間に届くすべての再試行に対して、同じ保存済みの結果を再生する -- 元の試行が死んだとみなされたあとに届く再試行も含めて。この保証の正確な境界 -- 実際には完了していた mutation のあとに引き継ぎが起きるとどうなるか -- については、[pending 予約の引き継ぎをフェンシングする](#pending-予約の引き継ぎをフェンシングする) で扱う。
...
- キーの形式はサーバー側で制限する（このレシピでは印字可能な ASCII 文字 1〜255 文字を強制する。後述のハンドラを参照）。不正な形式や過大なキーで台帳テーブルを肥大化させられないようにするためだ。
- idempotency キーは認証された呼び出し元にスコープされ、グローバルではない。サーバーは常に、クライアントが指定したキーをサーバー側で導出した `principal`（セッションのアカウント ID、API キーの所有者）と組み合わせてから保存・検索する。そのため異なる 2 つのアカウントが同じキー値を送っても、それは別々の行になる -- 一方のアカウントのレスポンスがもう一方に再生されることも、あるアカウントの進行中の予約が別アカウントの同一キーをブロックすることも決してない。
- サーバーは完了したキーの保存済みレスポンスを、無期限にではなく、一定の期間だけ保持する -- このレシピのスイープは行を 24 時間保持する（[上限付きの機会的スイープ](#上限付きの機会的スイープ) 参照）。その期間を過ぎてから届いた再試行は新しい操作と区別がつかず、mutation が再実行される。実際の保持期間を API リファレンスに明記し、現実的に最も遅いクライアントの再試行より十分長い値を選ぶこと。
...
台帳全体を 1 つのテーブルが担う。`principal` はすべての行を認証された呼び出し元にスコープし、異なる 2 つのアカウントが互いのキーに衝突したり、ブロックしたり、再生したりできないようにする。`generation` は後述するフェンシングトークンであり、`status` は保存済みレスポンスを再生してよいかどうかを決める。
...
claim は、既存キーの有無を確認する `SELECT` のあとに見つからなければ `INSERT` する、という形ではなく、単一のアトミックなステートメントで行わなければならない。同じ `principal` と同じキーを持つ 2 つの同時リクエストは、どちらの `INSERT` も着地する前に両方がその `SELECT` を実行しうる -- どちらも「行がない」と見て、どちらも自分がオーナーだと判断し、どちらも mutation の実行へ進んでしまう。レースを閉じるには、チェックと挿入が同一の操作でなければならない。
...
`principal` は、サーバー側で導出される安定したアカウントまたはユーザーの識別子だ -- このコードが実行される前に、自前の認証ミドルウェアによって解決され、クライアントが制御できるヘッダーやクエリパラメータから取られることは決してない。これがあるからこそ台帳は複数アカウント間で安全に共有できる: 2 つの異なる `principal` が、メソッド・パス・ボディまで完全に同一のリクエストを送っても、一方が他方のレスポンスを再生するのではなく、それぞれ独立した claim になる。
...
`claim` が空だったということは、この `principal` はこのキーをすでに claim したことがあるということだ。他の何よりも先に、サーバーはこれが同じリクエストの**正当な再試行**なのか、それとも実質的に異なるリクエストへの**同じキーの再利用**なのかを判断しなければならない。誤ったリクエストに対して保存済みの結果を再生してしまうことや、すでに実行済みの mutation を再実行してしまうことは、どちらも偽陽性の拒否より悪い失敗だ -- だからフィンガープリントの比較は意図的に厳格にしてある。

フィンガープリントは、正規化した 4 つの入力に対する SHA-256 の 16 進ダイジェストだ。
...
- **mutation の内容を変えうるヘッダーの固定された許可リスト** -- `Content-Type`、API バージョンを示すヘッダー、エンドポイントが尊重する条件付きヘッダー、その他ハンドラが分岐に使うものすべて。許可リストにないヘッダーはフィンガープリントに一切影響しないので、無害な違い（トレース ID、ユーザーエージェント文字列）が同一の操作を誤ってキー再利用として検出することはない。許可リストに含まれるヘッダーが、同じメソッド・パス・ボディを持つ 2 つのリクエスト間で異なる場合、それらのリクエストは同じ操作ではない -- これがなければ、`Content-Type` が変わってもキーを使い回すと、クライアントが今はマルチパートで送っているものに対して JSON のレスポンスを再生してしまう。
- **生のボディのバイト列**、一度だけバイナリとして読み取り、受け取った通りにハッシュする -- `request.text()` は決して使わず、ハッシュ計算前に `JSON.parse` してから再び `JSON.stringify` することも決してしない。これは 2 つの別々の失敗モードに対する 1 つの解決策だ: `request.text()` はボディを UTF-8 としてデコードし、不正なバイト列（バイナリのペイロード、マルチパートのボディ、壊れたエンコーディング）はすべて同じ U+FFFD 置換文字に潰れてしまうため、別々のリクエストが同一のハッシュになり衝突しうる。そして [HMAC-SHA256 によるウェブフック署名](./webhook-signing.mdx)で扱ったのと同じ再シリアライズの罠もここに当てはまる。キーの順序、空白、数値の表現はすべてパーサーを経由する往復で変化しうり、バイト列を変えてしまう。すると本来同一の 2 つのリクエストが異なるハッシュ値になってしまう。`request.arrayBuffer()` を一度だけ読み取り、その生バイト列を直接ハッシュすれば、この両方の問題を避けられる。
...
**フェンシングの不変条件:** すべての予約は `generation` を持つ。新しいキーを claim すると `1` から始まる。古びた `pending` の行を引き継ぐと、それをアトミックにインクリメントし、新しい値を新しいオーナーへ渡す。completion の書き込み -- `UPDATE ... WHERE principal = ? AND key = ? AND generation = ? AND status = 'pending'` -- は、その書き込みが対象とする generation が行の現在の generation とまだ一致している場合にのみ受け入れられる。引き継ぎは必ず generation を進めるので、ある時点で「現在の」generation を持つオーナーは高々 1 者しかいない。したがって、何者が同時に mutation を実行していようと、成功しうる completion の書き込みは高々 1 回だけだ。
...
:::warning[閉じるのはレスポンスのレースであって、mutation 実行のレースではない]
generation フェンスが保証するのは、高々 1 者のオーナーのレスポンスだけが記録・再生されるということだ -- `runMutation()` 自体が高々 1 回しか実行されないことは保証しない。Worker A の副作用が実際には完了していた場合 -- カードへの課金が成功した、メールが送信された -- のに、A が `completed` の行を書き込む前に死んでしまうと、Worker B による引き継ぎは mutation を最初から再実行してしまう。TTL を過ぎた pending の行は「正当にまだ動いている」ものと区別がつかないからだ。台帳には「完了したが、それを報告できなかった」ケースを「まだ実行中」のケースと見分ける手段がない。

mutation の重複実行が許容できないなら、次のどちらかが成り立っていなければならない: 下流の mutation 自体が同じキーのもとで冪等であること（決済 API 自身の idempotency キーを受け付ける、同じ値で重複排除されるキュー送信など）、あるいはこのパターンが実際に提供する保証を正直に述べること -- **少なくとも 1 回の実行、正確に 1 回だけ記録・再生されるレスポンス** -- 「厳密に 1 回だけの実行」を謳うのではなく。
:::
...
一部のステータスコードはボディを一切持たないと定義されている: `204 No Content`、`205 Reset Content`、そしてエンドポイントが mutation を行うメソッドに対して条件付きセマンティクスをサポートするなら `304 Not Modified` もそうだ。Fetch の `Response` コンストラクタはこれをプラットフォームレベルで強制する -- これらのステータスのいずれかとともに null でないボディ文字列を渡すと `TypeError: Response with null body status cannot have body` が投げられる。D1 に `result.body` の実際の値をそのまま保存するのは問題ない（空文字列も立派なカラム値だ）が、保存済みまたは新規の結果から `Response` を組み立てるすべての箇所は、まずこのチェックを通してボディを渡さなければならない。
...
`completed` の行を再生するのはその鏡像だ -- mutation はなく、保存済みのカラムからレスポンスを再構築するだけになる。同じ null ボディのチェックをここにも適用する。
...
`updated_at < now - RETENTION_MS` は、保持期間を過ぎた古い `completed` の行と、一度も引き継がれなかった `pending` の行の両方を捕える -- 誰も見直さなかった予約は、TTL が失効したものとまったく同じくらい死んでいる。リクエストごとの小さく上限付きのバッチは、各スイープを安価で自己ペース型に保つ。バックログは、1 回のクエリで一気に片付けようとして CPU 時間の予算を吹き飛ばすのではなく、多数のリクエストにわたって自己解消していく。[Cron 起動の D1 ワークキュー](./cron-d1-queue.mdx) の上限付き `LIMIT` バッチと同じ形だ。

`RETENTION_MS` は、上記の [Idempotency-Key ヘッダーの契約](#idempotency-key-ヘッダーの契約) で約束した保持期間そのものだ -- これを変更するなら、その約束も更新すること。両者が食い違ってはならない: 広告している期間よりスイープが短いと、その期間の終わり近くで再試行するクライアントに対して保証が黙って破られる。広告している期間よりスイープが長いと、クライアントに当てにしないよう伝えた行を単に保持し続けるだけになる。
...
ここまでのすべての部品は 1 つのハンドラへと組み上がる。キーを検証し、claim し、ルックアップで分岐し、必要なら古びた予約を引き継ぎ、generation ごとにちょうど 1 回だけ mutation を実行し、フェンス付きの completion を書き込み、返す前に機会的にスイープする。`principal` は自前の認証ミドルウェアで解決して渡す -- クライアントが制御できるものから取ることは決してない。
...
このパターンは D1 の基礎を [D1（SQL データベース）](../storage/d1.mdx) と共有し、上限付きバッチと `ctx.waitUntil()` 駆動のハウスキーピングを [Cron 起動の D1 ワークキュー](./cron-d1-queue.mdx) と共有している。フィンガープリントが依拠する生バイト列のハッシュ計算の作法については [HMAC-SHA256 によるウェブフック署名](./webhook-signing.mdx) を参照。
