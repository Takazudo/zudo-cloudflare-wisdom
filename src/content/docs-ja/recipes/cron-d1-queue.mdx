...
1. **ティック #1 が T = 0 秒で発火する。** R を選び、確保し（`claimed_until = T + 45秒`）、`processRow(R)` を開始する -- Webhook チャネルの応答が遅いとしよう。
2. **T = 50 秒 -- Worker インスタンスが退去させられる**（CPU 上限、キャッチされない例外、再デプロイなど）。配信の途中、`processRow` が `notifications` へ書き戻す前だ。R には `claimed_until = T + 45秒` が残るが、これはすでに過去であり、`next_fire_at` / `error_count` はティックが始まる前のままだ。
...
4. **ティック #2 が R を確保し**（`claimed_until = T + 60秒 + 45秒`）、最初から再処理する。
