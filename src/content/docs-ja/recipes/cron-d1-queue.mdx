...
上のクエリは、各ティックがキューを独り占めしている前提だ。だが cron は、前のティックが終わっていようといまいと、厳密に毎分発火する -- あるティックが長引けば（上流チャネルが遅い、D1 が一瞬詰まる、期日の行がバーストする）、次のティックの `scheduled()` 起動は、前のティックがまだ `processRow()` の中にいる間に始まりうる。両方の起動が同じ SELECT を実行し、両方が同じ期日の行を見る。それを止めるものが何もなければ、両方が同じ行を処理し -- 配信し -- 二重に送ってしまう。

その対策は、D1 ストレージのドキュメントにある [フェンシングトークンによる確保の保護](../storage/d1.mdx#アトミックな重複排除と行の確保) と同じプリミティブだ: 1 つのティックが、何かをする前にアトミックに確保するセンチネルカラムを持たせ、同じ行を確保しようとした 2 つ目のティックが負けるようにする。
...
SELECT と行ごとの UPDATE の両方が `claimed_until` でフィルタしている -- SELECT は、すでに確保済みの行を候補リストから外すことで、混み合ったティックがそれらを再チェックするだけのクエリ枠を無駄にしないようにする。だが実際に所有権を決めるのは UPDATE の方だ。SELECT だけではそれができない: 重複する 2 つのティックは、どちらかが確保する前に、同じ未確保の行を両方とも SELECT で見てしまいうるからだ。UPDATE の WHERE 句こそが本物の compare-and-swap であり、どちらのティックの UPDATE が先に着地するかで勝敗が決まる。負けた方は `RETURNING` から行を受け取れない。

確保は同じステートメントの中でアトミックにインクリメントされた `claim_token` も配り、それを後続の `SELECT` で読み直すのではなく、このステートメント自身から直接受け取る -- D1 ストレージのドキュメントの [期限付きリース](../storage/d1.mdx#アトミックな重複排除と行の確保-期限付きリース) と同じ理由からだ。ほんの少しあとに実行した `SELECT` は、この claim 自身のトークンではなく、より新しい再確保のトークンを見てしまいかねない。`processRow` はそのトークンをすべての書き戻し（下の節にある、進める / バックオフ / デッドレターの各分岐）に持ち回り、それぞれを `claim_token` でフェンシングする。そうすれば、止まっている `processRow()` 呼び出しの下から行が再確保されても、古い確保者がその結果を上書きすることはない。同じ書き戻しは、成功時に `claimed_until = NULL` も設定するべきだ。そうすれば、行はもう不要になったリースの残りを待つのではなく、自分の次の `next_fire_at` が来た時点ですぐに確保可能になる。

:::warning[claimed_until は必ず期限付きのリースにする -- 無期限の未来の値にしてはいけない]
`claimed_until` は「今からリース期間ぶん先」に設定すること。「自分が解除するまでロック」を意味するつもりの、遠い未来の値にしてはいけない。Worker インスタンスは `processRow()` の途中で -- CPU 時間の上限、キャッチされない例外、再デプロイなど -- 確保を解除する機会もなく退去させられうる。無期限のセンチネルはその行を永遠に立ち往生させる。期限付きのリースなら、リースが切れるまでの間だけ立ち往生させ、次のティックが自動的に再確保する。
:::

`CLAIM_LEASE_MS = 45_000` を 60 秒周期の cron に対して設定し、行 R が期日を迎えているとする。

1. **ティック #1 が T = 0 秒で発火する。** R を選び、その瞬間に取ったタイムスタンプで確保し（`claimed_until = T + 45秒`、`claim_token` が `0` から `1` へ進む）、`processRow(R, claimToken=1)` を開始する -- Webhook チャネルの応答が遅いとしよう。
2. **T = 50 秒 -- Worker インスタンスが退去させられる**（CPU 上限、キャッチされない例外、再デプロイなど）。配信の途中、`processRow` が `notifications` へ書き戻す前だ。R には `claimed_until = T + 45秒`（すでに過去）、`claim_token = 1` が残り、`next_fire_at` / `error_count` はティックが始まる前のままだ。
3. **ティック #2 が T = 60 秒で発火する。** その SELECT は R を再び含める: `next_fire_at <= now` はまだ成立し、`claimed_until（T+45秒）< now（T+60秒）` -- リースはすでに切れているので、R は再び候補になる。
4. **ティック #2 が R を確保し**（`claimed_until = T + 60秒 + 45秒`、`claim_token` が `2` へ進む）、`claimToken = 2` で最初から再処理する。
5. **もしティック #1 の配信が、クラッシュの直前に実際には成功していたなら** -- Webhook 呼び出しは成功したが書き戻しが実行されなかった場合 -- ティック #2 の再処理は重複配信を送ってしまう。これは上で述べた部分成功の重複配信のルールと同じ危険性だ: リースは、動かなくなった行が見えなくなっている時間の長さを制限するだけであり、それ単体で配信を冪等にするわけではない。`CLAIM_LEASE_MS` は現実的な最悪ケースに対してタイトに保ち、このウィンドウを小さく保つこと。そして重複送信が許容できない場合は、リースだけに頼るのではなく、配信そのものを冪等にすること。
6. **もしティック #1 の Worker インスタンスが、退去させられたのではなく単に止まっていただけだったら** -- まだ生きていて `claimToken = 1` を持ったまま、ただ遅いだけだとする -- そしてティック #2 がすでに R を再確保し、場合によっては完了させたあとになって書き戻しを試みたとしても、その書き戻しのステートメントは `claim_token = 1` でフェンシングされている。行の `claim_token` はもう `2` になっているので、ティック #1 のすべての書き込みは 0 行にマッチし、黙って拒否される -- ティック #1 の古い結果が、ティック #2（あるいはさらに後のティック）が書き込んだものを上書きすることは決してない。
...
どの分岐も、前節の確保から返ってきた `claimToken` を `WHERE` 句に持ち回り、`meta.changes` を確認する。`processRow()` が自分のリースより長生きすると、この呼び出しがまだ実行中の間に、後続のティックが同じ行を再確保して `claim_token` をさらに進めてしまいうる。フェンシングがなければ、この古い呼び出しの書き戻しは `id` だけを条件に無条件で着地し、新しい確保者の結果を上書きしたり、自分のものではない `claimed_until` をクリアしたりしかねない。ここで `meta.changes === 0` になるのはまさにその意味だ: 行は自分たちの知らないところで先へ進んでしまったので、この結果はもう古く、適用してはならない。
...
`ctx.waitUntil()` にも依存する即時レスポンス型の Webhook パターンについては [Bot Worker パターン](./bot-worker.mdx) を参照。D1 の基礎については -- [複数ライターの並行制御](../storage/d1.mdx#複数ライターの並行制御) や [アトミックな重複排除と行の確保](../storage/d1.mdx#アトミックな重複排除と行の確保) を含め -- [ストレージのドキュメント](../storage/d1.mdx) を参照。
