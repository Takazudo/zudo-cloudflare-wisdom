...
- 未設定 / `false`：あらゆる失敗（DNS レコードなし、ステータス異常、リダイレクト、マーカー欠落）は警告を出して `0` で終了します——「まだ live ではない」として扱われ、壊れたデプロイとしては扱われません。
...
- **クリーンアップ用の `trap ... EXIT`** —— これにより、作成したレコードはスクリプトが成功しても、verify ステップで失敗しても、ジョブの `timeout-minutes` で kill されても削除されます。ハッピーパスだけではありません。
