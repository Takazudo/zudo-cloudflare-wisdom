...
`@takazudo/zudo-doc`'s component styles are static Tailwind class literals in the dist JS, and Tailwind v4's `@source` scan of pnpm-symlinked `node_modules` does not reliably generate them (so package-only classes like the sticky-header `top-0`/`z-50` would go missing). The package solves this itself: at build it generates `dist/safelist.css` — a single `@source inline(...)` of every class token its dist declares — and exports it as `./safelist.css`. `src/styles/global.css` pulls it in with `@import "@takazudo/zudo-doc/safelist.css";` (module-resolved). **It auto-syncs on every `@takazudo/zudo-doc` bump — no consumer-side generator, nothing to re-run or re-commit.** Two `@theme` tokens the package classes reference (`--spacing-vsp-3xs`, `--shadow-lg`) must stay defined in `global.css` or those utilities no-op. See `zudolab/zudo-doc#1993`/`#1994` (and the consumer-side history in `Takazudo/zudo-front-builder#884`).
