...
#   1. Format check (mdx)
#   2. Category metadata check (pure-Node, reads src/ + zfb.config.ts only)
...
# Pure-Node: reads src/ + zfb.config.ts only, no install and no build needed.
# Guards the silently-dropped category metadata failure mode — zudo-doc's
# loadCategoryMeta reads _category_.json with node:fs, which zfb's embedded-V8
# host resolves to a throwing proxy, so every sidecar is discarded with a green
# build. Neither zfb check, html-validate nor check-links can see that.
